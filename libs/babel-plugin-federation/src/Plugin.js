...
export default class Plugin {
  constructor(
    ...
    this.federation = typeof federation === 'undefined' ? false : federation;
    this.federationOnly = federation === 'only';
    this.federationRemoteName =
      typeof federationRemoteName === 'undefined' ? 'common' : federationRemoteName;
    this.types = types;
    this.pluginStateKey = `federationPluginState${index}`;
  }
  ...
  importMethod(methodName, file, pluginState) {
    if (!pluginState.selectedMethods[methodName]) {
      ...
      if (this.customStyleName) {
        const originStylePath = winPath(this.customStyleName(transformedMethodName, file));
        const stylePath =
          this.federation && !originStylePath.startsWith('.')
            ? join(this.federationRemoteName, originStylePath)
            : originStylePath;
        addSideEffect(file.path, `${stylePath}`);
      } else if (this.styleLibraryDirectory) {
        const originStylePath = winPath(
          join(this.libraryName, this.styleLibraryDirectory, transformedMethodName, this.fileName),
        );
        const stylePath = this.federation
          ? join(this.federationRemoteName, originStylePath)
          : originStylePath;
        addSideEffect(file.path, `${stylePath}`);
      } else if (style === true) {
  ...
  ImportDeclaration(path, state) {
    ...
    if (value === libraryName) {
      if (this.federationOnly) {
        node.source.value = join(this.federationRemoteName, value);
      } else {
  ...
  CallExpression(path, state) {
    ...
    if (this.federationOnly) {
      if (name === 'require' || type === 'Import') {
        node.arguments = node.arguments.map(arg => {
          return {
            ...arg,
            value:
              arg.value === libraryName ? `${this.federationRemoteName}/${arg.value}` : arg.value,
          };
        });
      }
      return;
